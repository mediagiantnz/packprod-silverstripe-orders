...
export const handler = async (event) => {
  ...
  try {
    ...
    // Check for duplicate based on email + clientID (if email exists)
    if (customerData.email) {
      ...
      if (duplicateResult.Items && duplicateResult.Items.length > 0) {
        console.log(`Duplicate contact found for email: ${customerData.email}`);
        return {
          statusCode: 200,
          body: JSON.stringify({
    ...
    // Generate contact ID
    const contactID = customerData.contact_id || customerData.contactID || uuidv4();
    ...
    // Prepare the complete contact item with order data
    const contactItem = {
      contactID: contactID,
      clientID: clientID,
    ...
    try {
      ...
      console.log(`Successfully imported contact with order data: ${contactID}`);
    ...
    console.error("Error importing Packaging Products contact with order: ", error);
    return {
      statusCode: 500,
      body: JSON.stringify({
        message: "Failed to import Packaging Products contact with order data",
        error: error.message,
        stack: error.stack
      }),
      headers,
